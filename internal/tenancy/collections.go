...
func ensureInvitesCollection(app *pocketbase.PocketBase, options Options) error {
	...
	// FIXED: token is auto-generated by hook, so not required from user
	collection.Fields.Add(&core.TextField{Name: "token", Required: false, Max: 64})
	
	// FIXED: expires_at is auto-set by hook, so not required from user
	collection.Fields.Add(&core.DateField{Name: "expires_at", Required: false})
	...
	// FIXED: invited_by is optional (superuser/admin may not have auth user)
	// The hook will set this when possible, but it's not required
	collection.Fields.Add(&core.RelationField{
		Name:         "invited_by",
		Required:     false, // Optional: auto-set by hook when auth user is available
		MaxSelect:    1,
		CascadeDelete: true, // If inviter is deleted, remove their pending invites
		CollectionId: usersCollection.Id,
	})
